...
public class GuiDemo extends JFrame{
	...
	public GuiDemo() {
		...
		drawPanel.getTextItem().setText(// Step 7: Personalize Your Program
				"Life is 10%\n" +
				"      what happens to you,\n" +
				"And 90% of how you\n" +
				"      react to it."
		...
		// Refer to step 2: Add toolbar north of image (above the drawing area) to choose background image
		content.add( makeBackgroundToolBar(true), BorderLayout.NORTH );
		...
		//Refer to step 1: add menu to top bar (above drawing area) 
	...
	/** Step 2: Make a Toolbar
	 * Create the "Background" toolbar at the top of the window, 
	 * using objects of type ChooseBackgroundAction,
	 * 
	 */
	private JToolBar makeBackgroundToolBar(boolean horizontal) {
		...
		toolbar.addSeparator();
		toolbar.add(gradientOverlayCheckbox);
	...
	private class ChooseBackgroundAction extends AbstractAction {
		...
		ChooseBackgroundAction(String text) {
			...
			/*
			 * Step 4: ImageIcon from Scratch
			 * add background color and text to Custom icon (button)
			 */

			if (text.equals("Color...")) {
				putValue(Action.SHORT_DESCRIPTION, "<html>Use a solid color for background<br>instead of an image.</html>");
			BufferedImage Col = new BufferedImage(92,40,BufferedImage.TYPE_INT_ARGB);
			Graphics g = Col.createGraphics();
			g.setColor(Color.BLUE);
			g.fillRect(0,0,92,40);
			g.setColor(Color.GREEN);
			g.drawString("Choose Color",5,20);
			g.dispose();
			putValue(Action.SMALL_ICON, new ImageIcon(Col));
			putValue(Action.SHORT_DESCRIPTION, "Choose a Solid Background Color instead of Image"); // tooltip
			}
			
			/*
			 * Step 3: Adding an Icon to an Action
			 * add icon image to Custom icon (button)
			 */
